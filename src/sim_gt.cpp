...
sim_one_gt::sim_one_gt ( SimulationParameters* sim_param, action_board* simulation_jobs): parameters_(sim_param), simulation_jobs_ (simulation_jobs) {
	...
	for ( size_t node_i=0;node_i<my_Net.Net_nodes.size();node_i++){
	...
	for ( size_t i=0;i < my_Net.Net_nodes.size();i++){
	...
	for ( size_t sp_node_i=0;sp_node_i < my_Net.Net_nodes.size();sp_node_i++){
		if (!my_Net.Net_nodes[sp_node_i].tip_bool){
			...
			for ( size_t gt_node_i=0;gt_node_i<my_gt_coal_unit.Net_nodes.size();gt_node_i++){
	...
	if (sim_num_gener_bool){
		for ( size_t gt_node_i=0;gt_node_i<my_gt_num_gener.Net_nodes.size();gt_node_i++){
			Node* new_node_ptr=NULL;
	...
	while (remaining_sp_node.size()>0){
		...
		if (my_Net.Net_nodes[node_i].rank==rank_i){
			for ( size_t child_i=0;child_i<my_Net.Net_nodes[node_i].child.size();child_i++){
				if (my_Net.Net_nodes[node_i].child[child_i]->parent1->label==my_Net.Net_nodes[node_i].label){
					for ( size_t child_i_contains_gt_node1_i=0;child_i_contains_gt_node1_i<my_Net.Net_nodes[node_i].child[child_i]->Net_node_contains_gt_node1.size();child_i_contains_gt_node1_i++){
					...
					for ( size_t child_i_contains_gt_node1_i=0;child_i_contains_gt_node1_i<my_Net.Net_nodes[node_i].child[child_i]->Net_node_contains_gt_node2.size();child_i_contains_gt_node1_i++){
	...
	if ( this->simulation_jobs_->sim_num_mut_bool ||  this->simulation_jobs_->Si_num_bool){
		...
		total_brchlen=0;
		for ( size_t gt_node_i=0;gt_node_i<mt_tree.Net_nodes.size();gt_node_i++){
			Node* new_node_ptr=NULL;
...
void sim_one_gt::build_gt_string_mut_unit(double mutation_rate){
	...
	for ( size_t gt_node_i=0;gt_node_i<gt_mut_unit.Net_nodes.size();gt_node_i++){
		Node* new_node_ptr=NULL;
...
string pop_size_string /*! Network in extended newick form, branch lengths are the population sizes*/){
	...
	for ( size_t node_i=0;node_i<sp_num_gener_net.Net_nodes.size();node_i++){
		Node* new_node_ptr=NULL;
...
string rewrite_pop_string_by_para_string(
	...
	for ( size_t node_i=0;node_i<para_net_check.Net_nodes.size();node_i++){
		Node* new_node_ptr=NULL;
